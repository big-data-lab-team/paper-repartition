...
Two pieces of information are required to know the worst case memory consumption of the keep algorithm: what is the maximum number of each overlap area/volume we will have to keep in memory during the algorithm and what is the maximum size that those overlap areas/volumes can have. The maximum number of overlap to keep in memory is defined by the buffer order. \\
...
\begin{itemize}
  ...
  \item $F_1 + n_j(F_2 + F_3) + F_4(N_s-1)$
\end{itemize}
...
Finally, the maximum amount of memory needed is: .
