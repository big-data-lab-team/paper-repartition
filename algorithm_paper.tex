...
Therefore, the buffer order defines the maximum amount of extra data we will have to keep in memory.
...
% figure for the paragraph below:
This naive algorithm computes the overlap size in each dimension and returns an array of indices representing the dimensions, ordered by decreasing overlap size.
If there is no overlap in a given dimension, then the overlap size is 0.
The idea is to read the buffers in the direction where the overlap is the most important first, then read those columns of buffers in the order of the second dimension where the overlap is the biggest, etc.
...
We present the formulas for the 3D case, but it can be extended to the N dimensional case.
Let us say that the ordering is $[k,j,i]$. We keep at a maximum
\begin{itemize}
  \item one time the overlap in the first dimension of the buffer order: $f_1$.
  \item $n_k-1$ times the overlap in the second dimension of the buffer order: $(n_k-1)(f_2+f_3)$.
  \item $F_1 + n_j(F_2 + F_3) + F_4(N_s-1)$ % nani? cannot use f1 and F1 at same time
\end{itemize}

% TODO: f_1 etc may not be complete

\subsection{Overlap sizes in 3D}
...
Let us define the amount of extra data to be stored in each case.
...
\begin{itemize}
  ...
  \begin{itemize}
    ...
    \item Size of the b area: $f_2 = C_k(x)C_j(y)$ % refaire en fonction du buffer a la place
    \item Size of the c area: $f_3 = C_j(y)(B_k-C_k(x))$
  \end{itemize}
\end{itemize}

% figure of 3D overlap size with nomenclature or recylcle previous 3D overlap sizes, with d1 and d2

Then, in 3D, the amount of extra data is:
\begin{itemize}
  \item $e = (f_1+f_2+f_3)B_i$ while $B_i < d_1$ (see Figure x)
  \item $e = (f_1+f_2+f_3)d_1 + B_jB_k(B_i-d_1)$ while $B_i-d_1 < C_i(z)$
  \item $e = (f_1+f_2+f_3)d_1 + B_jB_kC_i(z)$, else
\end{itemize}
