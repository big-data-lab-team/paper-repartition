...
of candidate read shapes as follows: starting from $\hat r$ = ($\hat r_0$,
$\hat r_1$, $\hat r_2$), we evaluate candidate read shapes sorted by
decreasing $X_1$ value, then by decreasing $X_0$ value. We stop the search
when the number of generated seeks doesn't reduce for 10 iterations after
the current best read shape was found (read shape leading to the current minimum
seek number and respecting the memory constraint).
...
large array, the 6 disks were used as the capacity of a single disk was not
enough to accomodate the input or the output blocks. We used the disks in
round robin when writing the input blocks before the experiment and when
writing the output blocks during the experiment.
...
Speed-up is only observed when baseline requires $\mathbb{O}(10^8)$ seeks
(configurations 1-5): lower amounts of seek don't seem to have an effect on
...
allocated to the reprartitioning. For $m=275~GiB$, repartitioning times of
...
(Alg.~\ref{alg:keep}) and cache management (data copy to/from cache in
