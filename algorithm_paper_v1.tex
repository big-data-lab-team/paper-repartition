...
A baseline algorithm for the resplit task is to load one input file at a
time. It is equivalent to computing the buffers positions in
Algorithm~\ref{algo:generalresplit} using a buffer shape equal to $I$ \tristan{ok but in which order?}.
...
files match, a considerable amount of seeks will occur. Also, an overlap in the
...
\tristan{Algo 2 is difficult to read, a formula would be better.}
\begin{algorithm}
