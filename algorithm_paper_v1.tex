...
\begin{table}[ht]
  \centering
  \caption{\tristan{indiquer les buffer shapes (R) égales à $\hat R$}Description of the different buffer shapes used for each run by the keep algorithm, depending on the amount of RAM available. The last column gives the maximum RAM consumption estimated by the keep algorithm before running (at buffer selection time).}
