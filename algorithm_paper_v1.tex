...
\begin{equation}
  s_b = A_0 A_1 c_2 +
      A_0 c_1 m_2 +
      c_0 m_1 m_2
       + n_i + \mathcal{T}_0\mathcal{T}_1\mathcal{T}_2 \label{eq:seeks-baseline}
\end{equation}
with $ \mathcal{T}_d = \vert \mathcal{R}_d \cup \mathcal{O}_d \vert$
...
\begin{figure}
  ...
  \caption{Division of a read block into write blocks. Write blocks are merged using the scheme in Table~\ref{tab:fusion}. The blue block represents $F_0$.}
  \label{fig:nomenclature_overlaps}
  \end{figure}

  \tristan{what is F0 in fig 3, how is it defined?}
...
Finally, we had to copy data to the cache instead of reading directly into
it. \tristan{Timothée, je ne comprends pas la fin de ce paragraphe, peux-tu
mieux expliquer stp?} This is due to our formulation of the re-partition
problem which requires us to be able to predict the amount of memory
consumed by our algorithm to ensure that the limit $m$ will be respected.
Python uses a custom garbage collector which manages the release of our
data structures and prevents us to force the release of some data parts. We
found that copying the data to the cache enabled us to stay below the
...
repetition for the large array. The page cache was flushed at the begining
of each repetition.
...
\mytodo{x} on average. In configurations 1 to 5 in particular, baseline
...
\keep heuristic provides speed up factors of up to \mytodo{x} for
$m$=275~GiB, \mytodo{x} for $m$=8.6~GiB, and \mytodo{x} for $m$=4.3~GiB.
...
\tristan{Timothée, pourrais-tu exporter toutes les figures en pdf plutot qu'en png ? Les formats bitmap 
causent de l'aliasing quand on change l'échelle.}

\tristan{pourrais-tu ajouter une grille verticale sur la Figure 4, pour qu'on puisse mieux voir les valeurs ?}
\begin{figure*}[h]
  \centering
...
% Page cache
The memory page cache provided by the Linux kernel plays a significant
role in the repartitioning problem when the amount of memory increases.
When repartitioning a 85.75~GiB array with 275~GiB of working memory, 
all the output blocks are
written to memory and asynchronously flushed to disk, bringing
...
interesting to develop a repartitioning algorithm aware of this reality.
