...
\timothee{dans intro mettre juste ordre des octets et juste dire hdf5 utilise, nifti utilise}
...
\subsection{Dask}
\timothee{mieux expliquer d'où ça vient, parler de array chunking, faire le lien avec scipy bien pour faire la  science api dask array}
...
Now that the best buffer shape can be computed, we can express the \texttt{getBufferPositions}
function (Algorithm~\ref{algo:generalrechunk}) in Algorithm~\ref{algo:getbuffersposition}.

\begin{algorithm}
  \caption{Pseudocode of the \texttt{getBufferPositions} function}
  \label{algo:getbuffersposition}
  \begin{algorithmic}[1]
  \STATE $\Omega_{max}$ $\leftarrow$
...
\begin{algorithm}
  ...
  \begin{algorithmic}[1]
    ...
    \STATE \textbf{Outputs:} the buffer shape $B$, the index list of the remainder volumes to keep
