...
(I_i, I_j, I_k)$. Our goal is to rechunk the input blocks into output
...
that minimizes the number of seeks done by Algorithm~\ref{algo:generalrechunk}
...
A lower bound on the number of seeks for the rechunk task is $n_i + n_o$,
...
\begin{algorithm}
  ...
  \begin{algorithmic}[1]
    ...
    \FOR{buffer in buffersList}
      ...
      \FOR{(outFileIndex, data) in getDataToWrite(cache, policy)}
        \STATE write(outFiles[outFileIndex], data)
      \ENDFOR
    \ENDFOR
