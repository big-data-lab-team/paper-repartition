...
health sciences, geology and astrophysics \tristan{a reference would be welcome}, new big data challenges have emerged
related to the processing of large amounts of data, including ultra-high resolution images. Big Brain, a brain model providing
...
clone \tristan{\'evite d'utiliser autant de guillemets}, and
...
"Task" is a tuple with a callable as first element \tristan{All good but
you should explain the goal of the paper before explaining Dask. This is
too specific to read when we don't know where the paper is going}.

We focus on the \texttt{dask.array} collection, a data structure designed for
multi-dimensional array processing using blocked algorithms. Moreover, although Dask has
...
local schedulers \tristan{the last two sentences in this paragraph look out of place. I don't think you should justify 
more on the use of Dask than ``it is popular''.}.

\subsection{The occurrence of seeks}
% explain storage order
For this study we manipulate multidimensional arrays stored in HDF5
\tristan{I think this is the correct capitalization for HDF5 but you should
check and use it consistently (define a Latex command if it helps)} files,
that is why \tristan{You should reverse the logic: you assume that files
are written in row-major order, which is consistent with HDF5 in
particular. Also, your work doesn't depend much on this assumption so I wouldn't put it so central in the paragraph. What is more important is that 
the storage layout can be arbitrary because this is the main originality of your work compared to other ones, including the reading club paper, etc
You should start this paragraph with this explanation, and then mention that for clarity we will assume row-major but without loss of generality.} we assume that files are written in row-major order (a.k.a
``C" ordering),
...
\tristan{This paragraph should come right after the one ending with ``incurs  millions of seeks''. Dask is less critical to the paper, 
it should be mentioned in the intro but after the issue with seeks.}
Retrieving sub-arrays from an array stored on disk incurs seeks in two situations: (1) when an array block
...
appropriate output files \tristan{The last 2 sentences should be integrated because the first one gives the impresssion that the algo 
reads a buffer then writes it, but in fact it reads a buffer, adds it to the cache, and then write the complete output files 
...
merge tasks \tristan{at this point we don't know what is a split or a merge task, 
you should start this paragraph with a sentence explaining that they are a particular case of rechunk}.
 To illustrate these strategies we shall apply it \tristan{what is ``it'' here? } to the split task
...
In either case one can easily \tristan{``easy" isn't a good word in a paper: it's never as easy for the reader as you think.} find cases in which the algorithm seeks in all
dimensions.
...
resplit task. The algorithm presented in this study can be seen as a smarter \tristan{``smart" isn't a good word either, 
you can use ``extension'' instead.}
...
time. It is equivalent to replace \texttt{getBuffersPositions} by $I$ \tristan{the function returns a list, it cannot return $I$} in
...
As one can see from the algorithm \tristan{no one cannot see a number of seeks directly from the algorithm. You should give a numerical example 
to illustrate that. Algorithm 2 is unclear: is it the implementation of getBuffersPositions for the baseline case? 
if so, why are the inputs different? Inputs aren't well defined. Couldn't you give a formula for the number of seeks?}, unless the dimensions of the input and output
...
\begin{itemize}
  ...
  \item A public implementation of this algorithm in Dask, in a library called Dask IO \tristan{I feel like the name of the library is too generic for what it currently does. Something like Dask Rechunk would be better}.
  \item A public implementation in Dask of the ``clustered" split and merge strategy described in~\cite{seqalgorithms}, in Dask IO too.
\end{itemize}
