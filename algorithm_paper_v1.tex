...
seeks $s$ created by the baseline depends on how write blocks overlap with
...
\[
  ...
  \mathcal{O}_d = \Big\{ iO_d, i \in \llbracket 1, A_d/O_d \rrbracket \Big\} 
\]
...
\]
\[
  C_{i, d} = \Big\{ w \in \mathcal{W}_d, (i-1)O_d < w < iO_d \Big\}
\]
The number of output block pieces along dimension $d$ is then:
\[
  c_d = \sum_{i=1}^{A_d/O_d} \vert C_{i, d} \vert + 1 - \delta_{\vert C_{i, d} \vert, 0}
\]
where $\vert$ . $\vert$ is the cardinality operator and $\delta$ is the Kronecker delta. 
The number of matching endings is:
\[
  m_d = \vert \mathcal{W}_d \cup  \mathcal{O}_d \vert  - c_d
  \]
Finally, the number of seeks created by the baseline algorithm is:
\[
  s = A_0 A_1 c_2 + A_0 c_1 m_2 + c_0 m_1 m_2 + n_i + n_o
\]

As can be seen from this equation, cuts along dimension 2 generate $\mathcal{O}(A_0 A_1)}$
...
\begin{table}
  \begin{tabular}{ll}
    $\mathcal{A}$: & 3D array to repartition \\
    A: & shape of $\mathcal{A}$: A = ($A_1$, $A_2$, $A_3$)\\
    I: & shape of input blocks: I = ($I_1$, $I_2$, $I_3$)\\
    O: & shape of output blocks: O = ($O_1$, $O_2$, $O_3$)\\
    R: & shape of read blocks: R = ($R_1$, $R_2$, $R_3$)\\
    W: & shape of write blocks: W = ($W_1$, $W_2$, $W_3$)\\
...
\begin{algorithm}
  ...
  \begin{algorithmic}[1]
    ...
    \FOR{readShape in candidates}
      \STATE writeBlocks, m' = intersection(readBlocks, outBlocks)
      \IF{m' > m}
...
14), and provided that the
memory constraint is respected, merges write blocks as much as allowed by the memory 
constraint (line 18), computes 
the number of seeks created by the solution (line
19), and selects the read shape that minimizes the number of seeks (line
20-22). Finally, a list of read blocks is generated from the  selected read shape (line
24), and the algorithm returns.

\subsection{Algorithm for computing the number of seeks (keep)}
The amount of seeks produced by the keep algorithm for a given read buffer shape
$B$ is computed as follows:
$$s = s_o + s_i$$
$s_i$ corresponds to the number of seeks produced while reading the input blocks and
$s_o$ corresponds to the number of seeks produced by writing into the output blocks.
In the baseline algorithm, for each read buffer of shape $B=I$, we write the
content into fixed shape outblocks that intersect the read buffer.
In the reading part of the keep algorithm, we do the exact inverse process:
reading a fixed buffer of data from one or more input block(s).
We therefore compute $n_i$ using the same methodology than for the baseline
algorithm.
For the writing part of the keep algorithm however, the write buffer's shape is
not fixed so we cannot use the same methodology.
The amount of seeks is computed directly from the list of write buffers.
Consider one output block.
There is a set of write buffers describing how the output block
will be written.
These write buffers are a partition of the output block, and therefore do not
cross the output block and are of maximum size the size of the output block.
This leads us to a simple observation which is that in a given dimension, if a write
buffer shape is smaller than the output block shape, then writing the write buffer
will produce seeks (Figure~\ref{fig:seeks_and_rowmajor}).
From this obervation, we can say that for a given write buffer shape $WB$:
\begin{itemize}
  \item $WB_k \neq O_k => WB_i*WB_j$ seeks.
  \item $WB_j \neq O_j => WB_i$ seeks.
  \item $WB_i \neq O_i => 1$ seeks.
\end{itemize}
Again, the seeks in dimension $k$ include the seeks in dimension $j$ and the ones
produced in $j$ include the seeks produced in $i$.
Algorithm~\ref{algo:write_seeks_keep_algorithm} describes the computation of $n_o$.
\begin{algorithm}
  \caption{Number of seeks produced by the write step of the keep algorithm}
  \label{algo:write_seeks_keep_algorithm}
  \begin{algorithmic}[1]
    \STATE \textbf{Inputs}
    \STATE writeBuffers: list of the write buffers created by the keep algorithm.
    \STATE
    \STATE \textbf{Algorithm}
    \STATE $n_o \leftarrow \textrm{length(writeBuffers)}$ // number of file openings
    \FOR{writeBuffer in writeBuffers}
      \STATE $WB \leftarrow \textrm{shapeOf(writeBuffer)}$
      \IF{$WB_k \neq O_k$}
        \STATE $n_o += WB_i*WB_j$
      \ELSIF{$WB_k \neq O_k$}
        \STATE $n_o += WB_i$
...
\end{algorithm}
  ...
  \subsection{Algorithm for computing the max memory consumption (keep)}
