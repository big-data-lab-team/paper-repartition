...
shapes used for the experiment are shown on the Table~\ref{tab:exp2}.
...
the keep algorithm due to the difficulty to implement the special case where
$B_x<I_x$ (see section ``Implementation") \timothee{TODO}. In order to evaluate the
...
We did three sub-experiments on the keep algorithm. The first one consisted in
...
aggregate $B_x=\Lambda_x$. In particular, three interesting cases are
