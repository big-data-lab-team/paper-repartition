...
A re-partioning algorithm takes a
...
\begin{algorithm}
  ...
  \begin{algorithmic}[1]
    ...
    \FOR{$R$ in readShapes}
      ...
      \IF{mc > m}
      \STATE \textbf{continue} \COMMENT{Memory constraint cannot be fullfiled}
      \ENDIF
      \IF{$R$ == $\hat r$}
      \STATE bestReadBlocks = readBlocks
      \STATE \textbf{break} \COMMENT{$\hat r$ fullfills memory constraint}
      \ENDIF
...
enough to accomodate other formats in the future.
